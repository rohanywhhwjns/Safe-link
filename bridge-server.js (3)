...
async function dbLoad() {
  ...
  try {
    ...
    return data.record || { pairs: [], queuedMessages: {} };
  } catch(e) {
...
let lastUpdateId = 0;
let dbData = { pairs: [], queuedMessages: {} };
...
async function initDB() {
  ...
  if (dbData.queuedMessages) {
    ...
    console.log(`📂 Loaded queued messages for ${Object.keys(dbData.queuedMessages).length} users`);
  }
}
