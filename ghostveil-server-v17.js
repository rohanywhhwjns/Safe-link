...
async function dbSaveGroup(groupId, group) {
  ...
  await setDoc(doc(db, 'gv_groups', groupId), group);
}
async function dbLoadGroup(groupId) {
  const db = getFirestore();
  const snap = await getDoc(doc(db, 'gv_groups', groupId));
  return snap.exists ? snap.data() : null;
}
async function dbLoadGroupByCode(code) {
  const db = getFirestore();
  const snap = await getDoc(doc(db, 'gv_groupcodes', String(code).toUpperCase()));
...
async function dbDeleteGroupCode(code) {
  const db = getFirestore();
  await deleteDoc(doc(db, 'gv_groupcodes', String(code).toUpperCase()));
}
async function dbSaveGroupCode(code, groupId) {
  const db = getFirestore();
  await setDoc(doc(db, 'gv_groupcodes', String(code).toUpperCase()), { groupId, createdAt: Date.now() });
}
async function dbAddUserGroup(userId, groupId) {
  const db = getFirestore();
  const ref = doc(db, 'gv_usergroups', userId);
  const snap = await getDoc(ref);
  const cur = snap.exists ? (snap.data().groups || []) : [];
  if (!cur.includes(groupId)) cur.push(groupId);
  await setDoc(ref, { groups: cur });
}
...
async function dbListUserGroups(userId) {
  const db = getFirestore();
  const snap = await getDoc(doc(db, 'gv_usergroups', userId));
...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', async (data) => {
    ...
    if (msg.type === 'sync-contacts') {
      ...
      try {
        const contactsRef = doc(db, 'users', hashId(msg.userId), 'contacts', 'list');
        await setDoc(contactsRef, { ids: contactIds, updated: Date.now() }, { merge: true });
    ...
    if (msg.type === 'cloud-backup') {
      if (msg.accountId && msg.blob) {
        ...
        ws.send(JSON.stringify({ type: 'cloud-backup-saved', at: Date.now() })); // ACK so the phone can show "backup done"
        console.log('☁️ Encrypted cloud backup stored (ciphertext only)');
      }
      return;
    }
