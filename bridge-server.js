...
const mediaCache = new Map(); // id -> { data: base64, expiry: timestamp }
const MEDIA_CACHE_TTL = 300000; // 5 minutes

function cacheMedia(base64Data) {
  const id = 'media_' + Date.now() + '_' + Math.random().toString(36).substr(2,8);
  mediaCache.set(id, { data: base64Data, expiry: Date.now() + MEDIA_CACHE_TTL });
  // Cleanup old entries
  if (mediaCache.size > 100) {
    const now = Date.now();
    for (const [key, val] of mediaCache) {
      if (val.expiry < now) mediaCache.delete(key);
    }
  }
  return id;
}

// ---- HTTP Server (for serving cached media) ----
const httpServer = http.createServer((req, res) => {
  res.setHeader('Access-Control-Allow-Origin', '*');
  
  if (req.url.startsWith('/media/')) {
    const id = req.url.replace('/media/', '');
    const item = mediaCache.get(id);
    if (item) {
      const base64Data = item.data;
      const commaIdx = base64Data.indexOf(',');
      const mime = base64Data.substring(5, commaIdx);
      const data = base64Data.substring(commaIdx + 1);
  ...
  res.end('Safelink Bridge Server');
});

httpServer.listen(PORT - 1, () => {
  console.log(`🌐 HTTP media server on port ${PORT - 1}`);
});
...
async function handleTelegramMessage(message) {
  ...
  if (slId) {
    ...
    // If mediaUrl is large base64, cache it and send a URL instead
    if (mediaUrl && mediaUrl.startsWith('data:')) {
      const mediaId = cacheMedia(mediaUrl);
      msgObj.mediaUrl = `https://safe-link-01l7.onrender.com/media/${mediaId}`;
    }
