...
async function handleTelegramMessage(message) {
  ...
  if (slId) {
    ...
    if (message.text) {
      ...
      // Telegram stickers are WebP images - check if it has an emoji equivalent
      if (message.sticker.emoji) {
        // Use the sticker's emoji directly
        content = ''; mediaType = 'sticker'; 
        mediaUrl = null;
        // Store emoji in fileName field for the app to display
        fileName = message.sticker.emoji;
      } else {
        // No emoji - try to get the WebP file (may not display in all browsers)
