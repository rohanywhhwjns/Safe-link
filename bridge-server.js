...
// E2E INVITES: persisted so invite codes survive server restarts/sleeps
// (free-tier Render sleeps after idle — in-memory codes used to vanish)
async function dbSaveE2EInvite(code, invite) {
  if (!db) return;
  try {
    const ref = db.collection('safelink').doc('e2eInvites');
    const snap = await ref.get();
    const data = snap.exists ? snap.data() : {};
    data[code] = invite;
    await ref.set(data);
  } catch(e) { console.error('db save e2e invite:', e.message); }
}
async function dbLoadE2EInvite(code) {
  if (!db) return null;
  try {
    const snap = await db.collection('safelink').doc('e2eInvites').get();
    if (snap.exists) {
      const inv = (snap.data() || {})[code];
      if (inv && Date.now() - inv.createdAt < 1800000) return inv;
    }
  } catch(e) { console.error('db load e2e invite:', e.message); }
  return null;
}
async function dbDeleteE2EInvite(code) {
  if (!db) return;
  try {
    const ref = db.collection('safelink').doc('e2eInvites');
    const snap = await ref.get();
    if (snap.exists) {
      const data = snap.data() || {};
      if (data[code]) { delete data[code]; await ref.set(data); }
    }
  } catch(e) { /* best effort */ }
}
...
const httpServer = http.createServer((req, res) => {
  ...
  // ---- GROUP INVITE LINKS: /g/CODE ----
  if (req.url.startsWith('/g/')) {
    (async () => {
      const code = decodeURIComponent(req.url.slice(3).split('?')[0]).toUpperCase().trim();
      let groupName = null, off = false;
      try {
        const found = await dbLoadGroupByCode(code);
        if (found && found.group) {
          if (found.group.linkActive === false) off = true;
          else groupName = found.group.name;
        }
      } catch (e) {}
      const body = off || !groupName
        ? '<div class="card"><div class="big">🙈</div><h1>Invite unavailable</h1><p>The group owner has turned off this invite link, or it is invalid.</p></div>'
        : '<div class="card"><div class="big">🛡️</div><h1>' + groupName.replace(/[^a-zA-Z0-9 .()\-]/g, '') + '</h1><p>You are invited to a GhostVeil <b>end-to-end encrypted group</b>.</p><div class="code">' + code + '</div><p class="steps">1. Install the GhostVeil app<br>2. Open it → New Chat → Join Group<br>3. Paste this code</p><button onclick="navigator.clipboard?navigator.clipboard.writeText(\'' + code + '\').then(function(){alert(\'Code copied!\')}):alert(\'Code: ' + code + '\')">📋 Copy invite code</button></div>';
      const html = '<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>GhostVeil Group Invite</title><style>body{margin:0;font-family:-apple-system,Roboto,sans-serif;background:#0d1117;color:#e6edf3;display:flex;align-items:center;justify-content:center;min-height:100vh}.card{max-width:420px;margin:20px;padding:32px 28px;background:#161b22;border:1px solid #30363d;border-radius:20px;text-align:center}.big{font-size:48px}.h1,h1{font-size:22px;margin:10px 0}.code{font-size:28px;font-weight:700;letter-spacing:3px;background:#21262d;border:1px dashed #4fb286;padding:14px;border-radius:12px;margin:14px 0}.steps{color:#8b949e;font-size:14px;line-height:1.7}button{width:100%;padding:12px;border:none;border-radius:12px;background:linear-gradient(135deg,#4a9eff,#2d7dd2);color:#fff;font-size:15px;font-weight:600;margin-top:10px}</style></head><body>' + body + '</body></html>';
...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', async (data) => {
    ...
    if (msg.type === 'register') {
      ...
      // deliver any pending group join requests to group owners
      (async () => {
        try {
          for (const gid of await dbListUserGroups(userId)) {
            const g = await dbLoadGroup(gid);
            if (!g) continue;
            const ownerId = g.creator || ((g.members[0] || {}).id);
            if (ownerId !== userId) continue;
            for (const p of (g.pending || [])) ws.send(JSON.stringify({ type: 'group-join-request', groupId: gid, groupName: g.name, requester: p }));
          }
        } catch (e) { /* non-fatal */ }
      })();
    ...
    // ---- Telegram Bridge: DISCONNECT (the reverse of pairing) ----
    if (msg.type === 'unpair-telegram') {
      const rawTg = safelinkToTgChat.get(ws.userId);
      if (rawTg) {
        tgChatToSafelink.delete(hashId(rawTg));
        safelinkToTgChat.delete(ws.userId);
        savePairings();
        console.log(`✂️ Telegram bridge disconnected for ${ws.userId}`);
      }
      ws.send(JSON.stringify({ type: 'telegram-unpaired' }));
      return;
    }
    ...
    if (msg.type === 'e2e-invite') {
      ...
      setTimeout(() => e2eInvites.delete(code), 1800000); // in-memory: 30 min
      dbSaveE2EInvite(code, { from: ws.userId, createdAt: Date.now() }); // persisted: survives restarts
    ...
    if (msg.type === 'e2e-accept') {
      let invite = e2eInvites.get(msg.code);
      if (!invite || Date.now() - invite.createdAt > 1800000) {
        invite = await dbLoadE2EInvite(msg.code); // may live in Firestore after a server restart
      }
      if (!invite) {
    ...
    if (msg.type === 'group-create') {
      groupMutate(async () => {
        ...
        const group = { name, code, creator: ws.userId, linkActive: true, approval: false, members: [{ id: ws.userId, name: String(msg.myName || '').slice(0, 30) }], createdAt: Date.now() };
    ...
    if (msg.type === 'group-join') {
      groupMutate(async () => {
        ...
        // INVITE LINK OFF: owner deactivated — nobody new can get in
        if (group.linkActive === false) { ws.send(JSON.stringify({ type: 'group-failed', reason: 'The group owner has turned OFF the invite link' })); return; }
        // APPROVAL MODE: request instead of instant join
        const ownerId = group.creator || ((group.members[0] || {}).id);
        if (group.approval === true && ws.userId !== ownerId) {
          group.pending = group.pending || [];
          if (group.pending.some(p => p.id === ws.userId)) { ws.send(JSON.stringify({ type: 'group-failed', reason: 'You already asked to join — waiting for the owner' })); return; }
          group.pending.push({ id: ws.userId, name: joinerName, at: Date.now() });
          await dbSaveGroup(gid, group);
          ws.send(JSON.stringify({ type: 'group-request-sent' }));
          const ows = clients.get(ownerId);
          if (ows && ows.readyState === WebSocket.OPEN) ows.send(JSON.stringify({ type: 'group-join-request', groupId: gid, groupName: group.name, requester: { id: ws.userId, name: joinerName } }));
          return;
        }
    ...
    if (msg.type === 'group-newcode') {
      groupMutate(async () => {
        ...
        if (group) {
          const ownerId = group.creator || ((group.members[0] || {}).id);
          if (ws.userId !== ownerId) { ws.send(JSON.stringify({ type: 'group-failed', reason: 'Only the group owner can change the code' })); return; }
          const code = crypto.randomBytes(4).toString('hex').toUpperCase();
    ...
    // ---- Group: owner turns invite link on/off ----
    if (msg.type === 'group-setlink') {
      groupMutate(async () => {
        const group = await dbLoadGroup(msg.groupId);
        if (!group) return;
        const ownerId = group.creator || ((group.members[0] || {}).id);
        if (ws.userId !== ownerId) { ws.send(JSON.stringify({ type: 'group-failed', reason: 'Only the group owner can do that' })); return; }
        group.linkActive = !!msg.active;
        await dbSaveGroup(msg.groupId, group);
        ws.send(JSON.stringify({ type: 'group-link-set', groupId: msg.groupId, active: group.linkActive }));
      });
      return;
    }

    // ---- Group: owner requires approval for new members ----
    if (msg.type === 'group-setapproval') {
      groupMutate(async () => {
        const group = await dbLoadGroup(msg.groupId);
        if (!group) return;
        const ownerId = group.creator || ((group.members[0] || {}).id);
        if (ws.userId !== ownerId) { ws.send(JSON.stringify({ type: 'group-failed', reason: 'Only the group owner can do that' })); return; }
        group.approval = !!msg.approval;
        await dbSaveGroup(msg.groupId, group);
        ws.send(JSON.stringify({ type: 'group-approval-set', groupId: msg.groupId, approval: group.approval }));
      });
      return;
    }

    // ---- Group: owner approves / rejects a join request ----
    if (msg.type === 'group-approve') {
      groupMutate(async () => {
        const group = await dbLoadGroup(msg.groupId);
        if (!group) { ws.send(JSON.stringify({ type: 'group-failed', reason: 'Group not found' })); return; }
        const ownerId = group.creator || ((group.members[0] || {}).id);
        if (ws.userId !== ownerId) { ws.send(JSON.stringify({ type: 'group-failed', reason: 'Only the group owner can do that' })); return; }
        const pending = (group.pending || []).find(p => p.id === msg.userId);
        if (!pending) { ws.send(JSON.stringify({ type: 'group-failed', reason: 'No pending request from that person' })); return; }
        group.pending = (group.pending || []).filter(p => p.id !== msg.userId);
        if (msg.allow) {
          if ((group.members || []).length >= MAX_GROUP_MEMBERS) { ws.send(JSON.stringify({ type: 'group-failed', reason: 'Group is full' })); return; }
          group.members.push({ id: pending.id, name: pending.name });
        }
        await dbSaveGroup(msg.groupId, group);
        if (msg.allow) {
          await dbAddUserGroup(pending.id, msg.groupId);
          const roster = await groupRoster(group);
          const joinerKey = (roster.find(r => r.id === pending.id) || {}).pubKey || null;
          for (const m of group.members) {
            if (m.id === ws.userId) continue;
            const mws = clients.get(m.id);
            if (mws && mws.readyState === WebSocket.OPEN) mws.send(JSON.stringify({ type: 'group-member-joined', groupId: msg.groupId, member: { id: pending.id, name: pending.name, pubKey: joinerKey } }));
          }
          const jws = clients.get(pending.id);
          if (jws && jws.readyState === WebSocket.OPEN) jws.send(JSON.stringify({ type: 'group-joined', groupId: msg.groupId, name: group.name, members: roster }));
          ws.send(JSON.stringify({ type: 'group-approved', groupId: msg.groupId, name: pending.name }));
        } else {
          const jws = clients.get(pending.id);
          if (jws && jws.readyState === WebSocket.OPEN) jws.send(JSON.stringify({ type: 'group-rejected', groupId: msg.groupId }));
          ws.send(JSON.stringify({ type: 'group-rejected-done', groupId: msg.groupId, name: pending.name }));
        }
      });
      return;
    }

    // ---- Group: any member fetches the invite code + link state ----
    if (msg.type === 'group-getcode') {
      groupMutate(async () => {
        const group = await dbLoadGroup(msg.groupId);
        if (!group) { ws.send(JSON.stringify({ type: 'group-failed', reason: 'Group not found' })); return; }
        if (!(group.members || []).some(m => m.id === ws.userId)) { ws.send(JSON.stringify({ type: 'group-failed', reason: 'You are not a member of that group' })); return; }
        const ownerId = group.creator || ((group.members[0] || {}).id);
        ws.send(JSON.stringify({ type: 'group-info', groupId: msg.groupId, code: group.code, name: group.name, linkActive: group.linkActive !== false, approval: group.approval === true, owner: ws.userId === ownerId, pending: (group.pending || []) }));
      });
      return;
    }
