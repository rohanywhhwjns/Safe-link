...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', async (data) => {
    ...
    if (msg.type === 'register') {
      ...
      // If DB not ready yet, wait and retry
    ...
    if (msg.type === 'get-sticker-pack') {
      try {
        ...
        if (data.ok && data.result && data.result.stickers) {
          ...
          // Fetch ALL stickers in parallel (much faster)
...
async function handleTelegramMessage(message) {
  ...
  if (slId) {
    ...
    if (message.text) {
      ...
      // Detect sticker type: static (webp), animated (tgs), or video (webm)
      const isAnimated = message.sticker.is_animated || false;
      const isVideo = message.sticker.is_video || false;
      
      if (isVideo) {
        // Video sticker (WebM) - browser can play this with <video>
        mediaUrl = await getTelegramFile(message.sticker.file_id);
        if (mediaUrl) { content = ''; mediaType = 'sticker'; stickerKind = 'video'; }
      } else if (isAnimated) {
        // Animated sticker (TGS/Lottie) - can't play in browser directly
        // Get thumbnail (static image) as fallback, and note it's animated
        ...
        // Static sticker (WebP) - displays as image
        if (message.sticker.thumb && message.sticker.thumb.file_id) {
          mediaUrl = await getTelegramFile(message.sticker.thumb.file_id);
        }
        if (!mediaUrl) {
